 ...
 */
  ...
  write : function ( layout, input_path, output_path, use_requirejs ){
    ...
    var tmp = thunder.html_to_text( src );
    var tpl = layout.replace( '__content__', tmp );

    if( use_requirejs == 'false' ){
  ...
  build : function ( layout, input_dir, output_dir, use_requirejs ){
    var self = this;
    ...
    files.forEach( function ( file ){
      ...
      if( self.regex.html.test( file )){
        ...
        self.write( layout, new_input_path, new_output_path, use_requirejs );
      }
    });
  },
